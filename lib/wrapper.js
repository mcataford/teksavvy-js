...
var TeksavvyAPIWrapper = function TeksavvyAPIWrapper(apiKey) {
  ...
  this._getTargetURL = function (format) {
    if (format === _wrapperConstants2.default.formats.USAGE_RECORDS) {
      return _wrapperConstants2.default.urls.USAGE_RECORDS;
    } else if (format === _wrapperConstants2.default.formats.USAGE_SUMMARY) {
      return _wrapperConstants2.default.urls.USAGE_SUMMARY;
    }
  };
  ...
  this._formatResponse = function (response, format) {
    ...
    var datapoints = response.data.value.map(function (datum) {
      ...
      if (format === _wrapperConstants2.default.formats.USAGE_SUMMARY) {
        var additionalProps = {
      ...
      } else if (format === _wrapperConstants2.default.formats.USAGE_RECORDS) {
  ...
  this.usageRecords = function () {
    ...
    var format = _wrapperConstants2.default.formats.USAGE_RECORDS;
    ...
    return _axios2.default.get(url, { headers: headers, params: params }).catch(function (err) {
      return _promise2.default.reject(err.response.statusText);
  ...
  this.usageSummaries = function () {
    ...
    var format = _wrapperConstants2.default.formats.USAGE_SUMMARY;
    var url = _this._getTargetURL(format);
    var headers = _this._getDefaultHeaders();
    var requestTime = Date.now();
    var params = operator ? _this._queryBuilder.compose(operator) : null;
